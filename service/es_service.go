...
type EsService interface {
	...
	GetAllIDs(ctx context.Context, includeTypes, excludeFTPinkAuthorities, includeAllAuthorities bool) chan EsIDTypePair
}
...
func (es *esService) GetAllIDs(ctx context.Context, includeTypes, excludeFTPinkAuthorities, includeAllAuthorities bool) chan EsIDTypePair {
	...
	go func() {
		...
		if excludeFTPinkAuthorities {
			...
			r = elastic.NewScrollService(es.elasticClient).
				Index(es.indexName).
				Query(elastic.NewBoolQuery().
					Must(elastic.NewTermsQuery("authorities", "TME", "Smartlogic"))).
				Sort("_doc", true).
				Size(1000).
				FetchSource(includeTypes)
		}
